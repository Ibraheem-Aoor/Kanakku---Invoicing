...
namespace Kanakku\Space;

use Exception;
use Illuminate\Http\Request;
use Kanakku\Http\Requests\DatabaseEnvironmentRequest;
use Kanakku\Http\Requests\MailEnvironmentRequest;
use Kanakku\Http\Requests\DiskEnvironmentRequest;
...
class EnvironmentManager
{
    ...
    {
        $oldDatabaseConnection ='DB_CONNECTION='.config('database.default');

        $newDatabaseConnection ='DB_CONNECTION='.$request->database_connection;

        if($request->has('database_username') && $request->has('database_password')) {
            if(env('DB_USERNAME') && env('DB_HOST')){
                $oldDatabaseHost ='DB_HOST='.config('database.connections.'.config('database.default').'.host');
                $oldDatabasePort ='DB_PORT='.config('database.connections.'.config('database.default').'.port');
                $oldDatabaseName ='DB_DATABASE='.config('database.connections.'.config('database.default').'.database');
                $oldDatabaseUserName ='DB_USERNAME='.config('database.connections.'.config('database.default').'.username');
                $oldDatabasePassword ='DB_PASSWORD='.config('database.connections.'.config('database.default').'.password')."\n\n";
            } else {
                $oldDatabaseName ='DB_DATABASE='.config('database.connections.'.config('database.default').'.database');
            }

                $newDatabaseHost ='DB_HOST='.$request->database_hostname;
                $newDatabasePort ='DB_PORT='.$request->database_port;
                $newDatabaseName ='DB_DATABASE='.$request->database_name;
                $newDatabaseUserName ='DB_USERNAME='.$request->database_username;
                $newDatabasePassword ='DB_PASSWORD='.$request->database_password;
        ...
        try {
            ...
            file_put_contents($this->envPath, str_replace($oldDatabaseConnection,$newDatabaseConnection,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace($oldDatabaseHost,$newDatabaseHost,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace($oldDatabasePort,$newDatabasePort,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace($oldDatabaseName,$newDatabaseName,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace($oldDatabaseUserName,$newDatabaseUserName,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace($oldDatabasePassword,$newDatabasePassword,file_get_contents($this->envPath)));


            file_put_contents($this->envPath, str_replace('APP_URL='.config('app.url'),'APP_URL='.$request->app_url,file_get_contents($this->envPath)));

            file_put_contents($this->envPath, str_replace('SANCTUM_STATEFUL_DOMAINS='.env('SANCTUM_STATEFUL_DOMAINS'),'SANCTUM_STATEFUL_DOMAINS='.$request->app_domain,file_get_contents($this->envPath)));


            file_put_contents($this->envPath, str_replace('SESSION_DOMAIN='.config('session.domain'),'SESSION_DOMAIN='.explode(':',$request->app_domain)[0],file_get_contents($this->envPath)));
    ...
    public function saveMailVariables(MailEnvironmentRequest $request)
    {
        ...
        try {
            ...
            file_put_contents($this->envPath, str_replace($mailData['old_mail_data'],$mailData['new_mail_data'],file_get_contents($this->envPath)));

            if($mailData['extra_old_mail_data']) {
                file_put_contents($this->envPath, str_replace($mailData['extra_old_mail_data'],$mailData['extra_mail_data'],file_get_contents($this->envPath)));
            } else {
                file_put_contents(
                    $this->envPath,
    ...
    public function saveDiskVariables(DiskEnvironmentRequest $request)
    {
        ...
        try {
            ...
            if(!$diskData['old_default_driver']){
                ...
                file_put_contents($this->envPath, str_replace($diskData['old_default_driver'],$diskData['default_driver'],file_get_contents($this->envPath)));
            }
            ...
            if(!$diskData['old_disk_data']){
                ...
                file_put_contents($this->envPath, str_replace($diskData['old_disk_data'],$diskData['new_disk_data'],file_get_contents($this->envPath)));
            }
