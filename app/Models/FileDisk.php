...
class FileDisk extends Model
{
    ...
    public static function validateCredentials($credentials, $disk)
    {
        ...
        try {
            ...
            \Storage::disk($prefix . $disk)->put($root.'kanakku_temp.text', 'Check Credentials');

            if(\Storage::disk($prefix . $disk)->exists($root.'kanakku_temp.text')) {
                $exists = true;
                \Storage::disk($prefix . $disk)->delete($root.'kanakku_temp.text');
            }
        } catch(\Exception $e) {
            $exists = false;
        }
